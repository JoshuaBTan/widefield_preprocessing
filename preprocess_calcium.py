...
from widefield_pipeline.registration_new import (load_allen_atlas, register_atlas_landmarks, 
                         ...
                         crop_atlas_to_fov, make_brain_mask_fixed, resample_timeseries,
                         resample_frame)
...
def run_pipeline(config_file="config.yaml"):
    ...
    blue_mc, b_shifts = motion_correction_rigid(ch['blue'], 
                                               ...
                                               return_shifts=True)
    red_mc, r_shifts = motion_correction_rigid(ch['red'], 
    ...
    red_ref = np.median(red_mc.astype(np.float32), axis=0)
    ...
    red_mc_oriented = np.rot90(red_mc, k=config["normalization"]["rotate"], axes=(1,2))
    ...
    if registration_direction == "atlas_to_mouse":
        # ============ ORIGINAL WORKFLOW: Atlas warped to mouse ============
        print("\n--- ATLAS → MOUSE workflow ---") 
        
        # Register template to data
        template_reg, tform = register_atlas_landmarks(
            mean_blue_oriented, 
            template, 
        ...
        # Register atlas to data
        print("Transforming atlas to data...")
        atlas_reg = warp(
            atlas,
        ...
        # Make Brain Mask in mouse space
        brain_mask = make_brain_mask_fixed(mean_blue_oriented, 
                                          stack_for_std=blue_mc_oriented, 
                                          atlas=atlas_reg,
        ...
        print("Hemodynamic ROI time series extracted")
        
        hemo_pixel_timecourses = {}
        for signal_name, signal_data in hemo_signals.items():
            data = signal_data.copy()  # Make a copy to avoid modifying original
            data[:, ~brain_mask] = np.nan  # Mask the data
            hemo_pixel_timecourses[signal_name] = data
        print("Hemodynamic pixel time series extracted")
        ...
        with open(config["output"]["red_ref"], "wb") as f:
            pickle.dump(red_ref, f)
        
        
    else:  # mouse_to_atlas
    # ============ NEW WORKFLOW: Mouse warped to atlas ============
        print("\n--- MOUSE → ATLAS workflow ---")
    
        # Make brain mask in MOUSE space first
        brain_mask_mouse = make_brain_mask_fixed(
            mean_blue_oriented,
            stack_for_std=blue_mc_oriented,
            method=config["brain_mask"]["method"],
            qc=config["brain_mask"]["qc"]
        )
        print("Brain mask creation successful (mouse space)")
    
        # Register: Mouse → Atlas
        mouse_in_atlas, tform = register_atlas_landmarks(
            mean_blue_oriented,
            template,
            config["atlas"]["num_points"],
            mode=config["atlas"]["method"],
        ...
        red_atlas = warp(red_ref, inverse_map=tform.inverse,
        ...
        print("Transforming brain mask to atlas space...")
        brain_mask_atlas = apply_transform_to_mask(
            brain_mask_mouse, tform, output_shape=template.shape
        )
        ...
        # --- Extract hemodynamic signals (in mouse space, then transform) ---
        print("Extracting hemodynamic signals...")
        hemo_signals = extract_hemodynamic_signals(
            red_mc_oriented, green_mc_oriented,
            method=config["hemodynamic_extraction"]["method"],
            baseline_frames=slice(*config["hemodynamic_extraction"]["baseline_frames"]),
            green_wavelength=config["wavelength"]["green"],
            red_wavelength=config["wavelength"]["red"],
        ...
        for signal_name, signal_data in hemo_signals.items():
            ...
            data_masked[:, ~brain_mask_mouse] = np.nan
            data_atlas = apply_transform_to_stack(
                data_masked, tform, output_shape=template.shape, order=1
            )
